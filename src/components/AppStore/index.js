...
class AppStore extends Component {
  state = {appCategory: tabsList[0].tabId, searchValue: ''}

  getAppByCategory = () => {
    const {appCategory, searchValue} = this.state
    const getFilteredList = appsList.filter(
      eachApp =>
        eachApp.category === appCategory &&
        eachApp.appName.toLowerCase().includes(searchValue.toLowerCase()),
    )
    return getFilteredList
  }

  displayAppsBySearchInput = event => {
    this.setState({searchValue: event.target.value})
  }

  onClickTabItem = id => {
    this.setState({appCategory: id})
  }

  render() {
    const showAppByCategory = this.getAppByCategory()
      ...
      <div className="app-store-container">
        ...
        <div className="input-container">
          <input
            type="search"
            className="input-box"
            onChange={this.displayAppsBySearchInput}
            placeholder="Search"
          />
          <img
            alt="search icon"
        ...
        <ul className="list-item-container">
          {tabsList.map(eachTabItem => (
            <TabItem
              ...
              isTabActive={eachTabItem.tabId === appCategory}
            />
          ))}
        </ul>
        <ul className="app-container">
          {showAppByCategory.map(eachAppItem => (
            <AppItem
              key={eachAppItem.appId}
              eachAppItem={eachAppItem}
